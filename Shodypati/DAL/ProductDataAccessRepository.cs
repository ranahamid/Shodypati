...
namespace Shodypati.DAL
{
    ...
    {    
        ...
        {
            Product entity = Db.ProductTbls.Where(x => x.Id == id).Select(x => new Product()
            {
                Id                                  =x.Id                                   ,
                ProductName_English                 =x.ProductName_English                  ,
                ProductName_Bangla                  =x.ProductName_Bangla                   ,
        ...
        public void Delete(int id)
        {
            ...
            if (query.Count() == 1)
            {
                ProductTbl entity = query.FirstOrDefault();
        ...
        {
            string key = string.Format("GetProductsByCategoriesList");
            ...
            else
            {
                items = Db.CategoryTbls.Select(x => new CategoryMobile()
                {
                    ...
                    products = Db.ProductTbls.Where(y => y.CategoryId == x.Id && y.Published==true).Select(y => new ProductMobile()
                    {
                        Id = y.Id,
                ...
                List<CategoryMobile> itemsWithoutProduct = new List<CategoryMobile>();
        ...
        {
            string key = string.Format("GetProductsByCategoriesListWeb");
            //List<CategoryMobile> items=new List<CategoryMobile>();
            List<CategoryMobile> items = CacheStorage.Get<List<CategoryMobile>>(key);
        ...
        {
            string key = string.Format("GetProductsBy");
            ...
            List<ProductMobile> items = CacheStorage.Get<List<ProductMobile>>(key);
            List<ProductMobile> itemsReturn = new List<ProductMobile>();

            if (items == null)
            {
                string key2 = string.Format("AllProducts");
                List<ProductMobile> itemsAll = CacheStorage.Get<List<ProductMobile>>(key2);
        
                List<ProductMobile> items1 = new List<ProductMobile>();
                List<ProductMobile> items2 = new List<ProductMobile>();
                ...
                if(categoryId!=0)
                {
                     items1 = items.Where(y => y.CategoryId == categoryId).ToList();
                }
                if (merchantId != 0)
                {
                    items2 = items.Where(y => y.MerchantId == merchantId).ToList();
                }
