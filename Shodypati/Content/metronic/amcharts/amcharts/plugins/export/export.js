    ...
    AmCharts["export"] = function(chart, config) {
        var _this = {
                    ...
                    add: function(options) {
                        ...
                        method(cfg.url,
                            function(objects, options) {
                                var group = options !== undefined
                                    ? fabric.util.groupSVGElements(objects, options)
                                    : objects;
                                var ratio = false;

                                // RESCALE ONLY IF IT EXCEEDS THE CANVAS
                                if (group.height > _this.setup.fabric.height ||
                                    group.width > _this.setup.fabric.width) {
                ...
                for (i1 = 0; i1 < group.children.length; i1++) {
                    ...
                    if (childNode.tagName == "clipPath") {
                        ...
                        if (cfg.removeImages && _this.isTainted(props.source)) {
                            ...
                            fabric.Image.fromURL(props.source,
                                (function(props) {
                                    return function(img) {
                                        images.loaded++;
                        ...
                        // LOAD IMAGE MANUALLY; TO RERENDER THE CANVAS
                        fabric.Image.fromURL(childNode.getAttribute("xlink:href"),
                            function(img) {
                                images.loaded++;
                            });
                    }
            ...
            capture: function(options, callback) {
                ...
                if (_this.config.legend && _this.setup.chart.legend && _this.setup.chart.legend.position == "outside") {
                    var group = {
                        ...
                        legend: {
                            ...
                            width: _this.config.legend.width
                                ? _this.config.legend.width
                                : _this.setup.chart.legend.container.width,
                            height: _this.config.legend.height
                                ? _this.config.legend.height
                                : _this.setup.chart.legend.container.height
                        },
                        patterns: {},
                        clippings: {}
                    };
                ...
                if (_this.setup.chart.type == "stock") {
                    ...
                    if (_this.setup.chart.leftContainer) {
                        ...
                        padding.left = _this.setup.chart.leftContainer.offsetWidth +
                            (_this.setup.chart.panelsSettings.panelSpacing * 2);
                    }
                    if (_this.setup.chart.rightContainer) {
                        offset.width -= _this.setup.chart.rightContainer.offsetWidth;
                        padding.right = _this.setup.chart.rightContainer.offsetWidth +
                            (_this.setup.chart.panelsSettings.panelSpacing * 2);
                    }
                    if (_this.setup.chart.periodSelector &&
                        ["top", "bottom"].indexOf(_this.setup.chart.periodSelector.position) != -1) {
                        offset.height -= _this.setup.chart.periodSelector.offsetHeight +
                            _this.setup.chart.panelsSettings.panelSpacing;
                        padding[_this.setup.chart.periodSelector.position] += _this.setup.chart.periodSelector
                            .offsetHeight +
                            _this.setup.chart.panelsSettings.panelSpacing;
                    }
                    if (_this.setup.chart.dataSetSelector &&
                        ["top", "bottom"].indexOf(_this.setup.chart.dataSetSelector.position) != -1) {
                        offset.height -= _this.setup.chart.dataSetSelector.offsetHeight;
                        padding[_this.setup.chart.dataSetSelector.position] += _this.setup.chart.dataSetSelector
                            .offsetHeight;
                    }
                ...
                _this.setup.fabric = new fabric.Canvas(_this.setup.canvas,
                    _this.deepMerge({
                            width: offset.width,
                ...
                _this.setup.fabric.on("mouse:up",
                    function(e) {
                        // SELECT TARGET
                ...
                _this.setup.fabric.on("object:selected",
                    function(e) {
                        _this.drawing.buffer.isSelected = true;
                        _this.drawing.buffer.target = e.target;
                        _this.setup.fabric.isDrawingMode = false;
                    });
                _this.setup.fabric.on("selection:cleared",
                    function(e) {
                ...
                _this.setup.fabric.on("path:created",
                    function(e) {
                        var item = e.path;
                ...
                _this.setup.fabric.on("object:added",
                    function(e) {
                        var item = e.target;
                ...
                _this.setup.fabric.on("object:modified",
                    function(e) {
                        var item = e.target;
                ...
                _this.setup.fabric.on("text:changed",
                    function(e) {
                        var item = e.target;
                ...
                if (_this.drawing.buffer.enabled) {
                    _this.setup.wrapper.setAttribute("class",
                        _this.setup.chart.classNamePrefix + "-export-canvas active");
                ...
                for (i1 = 0; i1 < groups.length; i1++) {
                    var group = groups[i1];
                    var isLegend = _this.gatherClassName(group.parent,
                        _this.setup.chart.classNamePrefix + "-legend-div",
                        1);
                    var isPanel = _this.gatherClassName(group.parent,
                        _this.setup.chart.classNamePrefix + "-stock-panel-div");
                    var isScrollbar = _this.gatherClassName(group.parent,
                        _this.setup.chart.classNamePrefix + "-scrollbar-chart-div");
                    ...
                    if (_this.setup.chart.type == "stock" && _this.setup.chart.legendSettings.position) {
                        ...
                        if (["top", "bottom"].indexOf(_this.setup.chart.legendSettings.position) != -1) {
                            ...
                            // LEGEND; OFFSET
                            if (isLegend) {
                                offset.pY += _this.pxToNumber(isPanel.style.height) +
                                    _this.setup.chart.panelsSettings.panelSpacing;
                    ...
                    // ADD TO CANVAS
                    fabric.parseSVGDocument(group.svg,
                        (function(group) {
                            return function(objects, options) {
                                    ...
                                    if (g.paths[i1]) {
                                        ...
                                        if (String(g.paths[i1].clipPath).slice(0, 3) == "url") {
                                            ...
                                            if (group.clippings[PID]) {
                                                ...
                                                var transform = g.paths[i1].svg.getAttribute("transform") ||
                                                    "translate(0,0)";
                                                ...
                                                g.paths[i1].set({
                                                    clipTo: (function(mask, transform) {
                                                        return function(ctx) {
                                                            ...
                                                            ctx.rect(Number(transform[0]) * -1 + x,
                                                                Number(transform[1]) * -1 + y,
                                                                width,
                                                                height);
                                                        };
                                                    })(mask, transform)
                                ...
                                if (group.svg.parentNode && group.svg.parentNode.getElementsByTagName) {
                                    var balloons =
                                        group.svg.parentNode.getElementsByClassName(_this.setup.chart.classNamePrefix +
                                            "-balloon-div");
                                ...
                                if (group.svg.nextSibling && group.svg.nextSibling.tagName == "A") {
                                    var label = new fabric.Text(
                                        group.svg.nextSibling.innerText || group.svg.nextSibling.innerHTML,
                                        {
                        ...
                        function(svg, obj) {
                            ...
                            if (visibility == "hidden") {
                                ...
                                for (i1 = 0; i1 < attrs.length; i1++) {
                                    ...
                                    // EXCEPTION
                                    if (obj.classList.indexOf(_this.setup.chart.classNamePrefix + "-guide-fill") !=
                                        -1 &&
                                        !attrVal) {
                                        attrOpacity = 0;
            ...
            toPRINT: function(options, callback) {
                ...
                data.setAttribute("style", "max-height: 100%;  width: 100%;");
            ...
            handleDropbox: function(e) {
                if (_this.drawing.buffer.enabled) {
                    ...
                    if (e.type == "dragover") {
                        _this.setup.wrapper.setAttribute("class",
                            _this.setup.chart.classNamePrefix + "-export-canvas active dropbox");
                        ...
                        for (i1 = 0; i1 < _this.setup.chart.dataProvider.length; i1++) {
                            ...
                            if (dataItem[field]) {
                                for (i2 = 0; i2 < dataItem[field].length; i2++) {
                                    dataItem[field][i2][_this.setup.chart.categoryField] =
                                        dataItem[_this.setup.chart.categoryField];
            ...
            createMenu: function(list, container) {
                ...
                function buildList(list, container) {
                    ...
                    for (i1 = 0; i1 < list.length; i1++) {
                        ...
                        // FILTER; TOGGLE FLAG
                        if (["CSV", "JSON", "XLSX"].indexOf(item.format) != -1 &&
                            ["map", "gauge"].indexOf(_this.setup.chart.type) != -1) {
                            continue;
                            ...
                            if (_this.drawing.handler[action] instanceof Function) {
                                ...
                                item.click = (function(item) {
                                    return function() {
                                        ...
                                        this["to" + item.format](item,
                                            function(data) {
                                                if (item.action == "download") {
                                                    this.download(data,
                                                        item.mimeType,
                                                        [item.fileName, item.extension].join("."));
                                                }
                                            });
                                    };
                                })(item);
                        ...
                        a.addEventListener("click",
                            (function(callback, item) {
                                return function(e) {
                                    ...
                                    if ((item.action == "draw" ||
                                            item.format == "PRINT" ||
                                            (item.format != "UNDEFINED" && item.capture)) &&
                                        !_this.drawing.enabled) {
                        ...
                        if (item.icon) {
                            img.setAttribute("src",
                                (!item.ignore && item.icon.slice(0, 10).indexOf("//") == -1 ? chart.pathToImages : "") +
                                item.icon);
                            a.appendChild(img);
                        }
                        if (item.label) {
                ...
                _this.setup.menu.setAttribute("class",
                    _this.setup.chart.classNamePrefix +
                    "-export-menu " +
                    _this.setup.chart.classNamePrefix +
                    "-export-menu-" +
            ...
            delay: function(options, callback) {
                ...
                var menu = _this.createMenu([
                    {
                        label: _this.i18l("capturing.delayed.menu.label")
                            .replace("{{duration}}", AmCharts.toFixed(cfg.delay, cfg.precision)),
                ...
                t1 = setInterval(function() {
                        ...
                        if (diff <= 0) {
                            ...
                            label.innerHTML = _this.i18l("capturing.delayed.menu.label")
                                .replace("{{duration}}", AmCharts.toFixed(diff, 2));
                        }
                    },
                    10);
