        ...
        var write_num = (function make_write_num() {
            ...
            function write_num_flt(type, fmt, val) {
                ...
                if ((r = fmt.match(dec1)) !== null) {
                    o = rnd(val, r[1].length).replace(/^([^\.]+)$/, "$1." + r[1]).replace(/\.$/, "." + r[1])
                        .replace(/\.(\d*)$/,
                            function($$, $1) { return "." + $1 + fill("0", r[1].length - $1.length); });
                    return fmt.indexOf("0.") !== -1 ? o : o.replace(/^0\./, ".");
                }
                fmt = fmt.replace(/^#+([0.])/, "$1");
                if ((r = fmt.match(/^(0*)\.(#*)$/)) !== null) {
                    return sign +
                        rnd(aval, r[2].length).replace(/\.(\d*[1-9])0*$/, ".$1").replace(/^(-?\d*)$/, "$1.")
                ...
                if ((r = fmt.match(/^([0#]+)(\\?-([0#]+))+$/)) !== null) {
                    ...
                    return _strrev(_strrev(fmt.replace(/\\/g, "")).replace(/[0#]/g,
                        function(x) { return ri < o.length ? o[ri++] : x === "0" ? "0" : ""; }));
                }
                ...
                if ((r = fmt.match(/^00,000\.([#0]*0)$/)) !== null) {
                    ...
                    return val < 0
                        ? "-" + write_num_flt(type, fmt, -val)
                        : commaify(flr(val)).replace(/^\d,\d{3}$/, "0$&").replace(/^\d*$/,
                            function($$) { return "00," + ($$.length < 3 ? pad0(0, 3 - $$.length) : "") + $$; }) +
                        "." +
                        pad0(ri, r[1].length);
                }
                switch (fmt) {
            ...
            function write_num_int(type, fmt, val) {
                ...
                if ((r = fmt.match(/^(0*)\.(#*)$/)) !== null) {
                    return sign +
                        ("" + aval).replace(/\.(\d*[1-9])0*$/, ".$1").replace(/^(-?\d*)$/, "$1.")
                        .replace(/^0\./, r[1].length ? "0." : ".");
                }
                ...
                if ((r = fmt.match(/^00,000\.([#0]*0)$/)) !== null) {
                    return val < 0
                        ? "-" + write_num_int(type, fmt, -val)
                        : commaify("" + val).replace(/^\d,\d{3}$/, "0$&").replace(/^\d*$/,
                            function($$) { return "00," + ($$.length < 3 ? pad0(0, 3 - $$.length) : "") + $$; }) +
                        "." +
                        pad0(0, r[1].length);
                }
            ...
            while (i < fmt.length) {
                ...
                case "#":
                    ...
                    while ("0#?.,E+-%".indexOf(c = fmt[++i]) > -1 ||
                        c == "\\" && fmt[i + 1] == "-" && "0#".indexOf(fmt[i + 2]) > -1) o += c;
            ...
            for (; i < sector.length; i += 128) {
                ...
                if (o.type === 5) { /* root */
                    ...
                    if (sector_list[o.start] === undefined)
                        sector_list[o.start] =
                            get_sector_list(sectors, o.start, sector_list.fat_addrs, sector_list.ssz);
...
// TODO: CP remap (need to read file version to determine OS)
    var unescapexml = (function() {
        ...
        return function unescapexml(text) {
            var s = text + "";
            return s.replace(encregex, function($$) { return encodings[$$]; }).replace(coderegex,
                function(m, c) { return String.fromCharCode(parseInt(c, 16)); });
        };
    })();
    ...
    function escapexml(text) {
        var s = text + "";
        return s.replace(decregex, function(y) { return rencoding[y]; }).replace(charegex,
            function(s) { return "_x" + ("000" + s.charCodeAt(0).toString(16)).substr(-4) + "_"; });
    }
...
// matches <foo>...</foo> extracts content
    var matchtag = (function() {
        ...
        return function matchtag(f, g) {
            ...
            return (mtcache[t] =
                new RegExp("<(?:\\w+:)?" + f + '(?: xml:space="preserve")?(?:[^>]*)>([^\u2603]*)</(?:\\w+:)?' + f + ">",
                    (g || "")));
        };
    })();
    ...
    __hexlify = ___hexlify = function hexlify_(b, s, l) {
        return b.slice(s, (s + l)).map(function(x) { return (x < 16 ? "0" : "") + x.toString(16); }).join("");
    };
    var __utf8, ___utf8;
    ...
    if (has_buf) {
        ...
        __hexlify =
            function(b, s, l) { return Buffer.isBuffer(b) ? b.toString("hex", s, s + l) : ___hexlify(b, s, l); };
...
/* from js-xls */
    ...
    var __unhexlify = typeof Buffer !== "undefined"
        ? function(s) { return Buffer.isBuffer(s) ? new Buffer(s, "hex") : ___unhexlify(s); }
        : ___unhexlify;
...
/* [MS-XLSB] 2.1.7 Part Enumeration */
    ...
    var CTYPE_XML_ROOT = writextag("Types",
        null,
        {
            'xmlns': XMLNS.CT,
            'xmlns:xsd': XMLNS.xsd,
...
/* [MS-OSHARED] 2.3.3.2.[1-2].1 (PIDSI/PIDDSI) */
    ...
    var CORE_PROPS_XML_ROOT = writextag("cp:coreProperties",
        null,
        {
...
/* [MS-OSHARED] 2.3.3.2.[1-2].1 (PIDSI/PIDDSI) */
    ...
    var EXT_PROPS_XML_ROOT = writextag("Properties",
        null,
        {
    ...
    function write_ext_props(cp, opts) {
        ...
        o[o.length] = (W("HeadingPairs",
            W("vt:vector",
                W("vt:variant", "<vt:lpstr>Worksheets</vt:lpstr>") + W("vt:variant", W("vt:i4", String(cp.Worksheets))),
                { size: 2, baseType: "variant" })));
        o[o.length] = (W("TitlesOfParts",
            W("vt:vector",
                cp.SheetNames.map(function(s) { return "<vt:lpstr>" + s + "</vt:lpstr>"; }).join(""),
...
/* 15.2.12.2 Custom File Properties Part */
    ...
    var CUST_PROPS_XML_ROOT = writextag("Properties",
        null,
        {
...
/* [MS-OFFCRYPTO] 2.3.7.2 Binary Document XOR Array Initialization */
    var crypto_CreateXorArray_Method1 = (function() {
        ...
        var XorMatrix = [
            0xAEFC, 0x4DD9, 0x9BB2, 0x2745, 0x4E8A, 0x9D14, 0x2A09, 0x7B61, 0xF6C2, 0xFDA5, 0xEB6B, 0xC6F7, 0x9DCF,
            0x2BBF, 0x4563, 0x8AC6, 0x05AD, 0x0B5A, 0x16B4, 0x2D68, 0x5AD0, 0x0375, 0x06EA, 0x0DD4, 0x1BA8, 0x3750,
            0x6EA0, 0xDD40, 0xD849, 0xA0B3, 0x5147, 0xA28E, 0x553D, 0xAA7A, 0x44D5, 0x6F45, 0xDE8A, 0xAD35, 0x4A4B,
            0x9496, 0x390D, 0x721A, 0xEB23, 0xC667, 0x9CEF, 0x29FF, 0x53FE, 0xA7FC, 0x5FD9, 0x47D3, 0x8FA6, 0x0F6D,
            0x1EDA, 0x3DB4, 0x7B68, 0xF6D0, 0xB861, 0x60E3, 0xC1C6, 0x93AD, 0x377B, 0x6EF6, 0xDDEC, 0x45A0, 0x8B40,
            0x06A1, 0x0D42, 0x1A84, 0x3508, 0x6A10, 0xAA51, 0x4483, 0x8906, 0x022D, 0x045A, 0x08B4, 0x1168, 0x76B4,
            0xED68, 0xCAF1, 0x85C3, 0x1BA7, 0x374E, 0x6E9C, 0x3730, 0x6E60, 0xDCC0, 0xA9A1, 0x4363, 0x86C6, 0x1DAD,
            0x3331, 0x6662, 0xCCC4, 0x89A9, 0x0373, 0x06E6, 0x0DCC, 0x1021, 0x2042, 0x4084, 0x8108, 0x1231, 0x2462,
            0x48C4
        ];
...
/* 18.8 Styles CT_Stylesheet*/
    ...
    var STYLES_XML_ROOT = writextag("styleSheet",
        null,
        {
            'xmlns': XMLNS.main[0],
            'xmlns:vt': XMLNS.vt
...
/* 18.7.3 CT_Comment */
    function parse_comments_xml(data, opts) {
        ...
        var authors = [];
        var commentList = [];
        data.match(/<(?:\w+:)?authors>([^\u2603]*)<\/(?:\w+:)?authors>/)[1].split(/<\/\w*:?author>/).forEach(
            function(x) {
                if (x === "" || x.trim() === "") return;
                authors.push(x.match(/<(?:\w+:)?author[^>]*>(.*)/)[1]);
            });
        (data.match(/<(?:\w+:)?commentList>([^\u2603]*)<\/(?:\w+:)?commentList>/) || ["", ""])[1]
            .split(/<\/\w*:?comment>/).forEach(function(x, index) {
                if (x === "" || x.trim() === "") return;
        ...
        if (fillid)
            try {
                ...
                if (p.s.fgColor && p.s.fgColor.theme) {
                    p.s.fgColor.rgb = rgb_tint(themes.themeElements.clrScheme[p.s.fgColor.theme].rgb,
                        p.s.fgColor.tint || 0);
                    if (opts.WTF) p.s.fgColor.raw_rgb = themes.themeElements.clrScheme[p.s.fgColor.theme].rgb;
                }
                if (p.s.bgColor && p.s.bgColor.theme) {
                    p.s.bgColor.rgb = rgb_tint(themes.themeElements.clrScheme[p.s.bgColor.theme].rgb,
                        p.s.bgColor.tint || 0);
...
/* 18.3 Worksheets */
    ...
    var WS_XML_ROOT = writextag("worksheet",
        null,
        {
            'xmlns': XMLNS.main[0],
            'xmlns:r': XMLNS.r
...
/* 18.2 Workbook */
    ...
    var WB_XML_ROOT = writextag("workbook",
        null,
        {
            'xmlns': XMLNS.main[0],
            //'xmlns:mx': XMLNS.mx,
    ...
    function write_wb_xml(wb, opts) {
        ...
        for (var i = 0; i != wb.SheetNames.length; ++i)
            o[o.length] = (writextag("sheet",
                null,
                { name: wb.SheetNames[i].substr(0, 31), sheetId: "" + (i + 1), "r:id": "rId" + (i + 1) }));
        o[o.length] = "</sheets>";
        if (o.length > 2) {
            o[o.length] = "</workbook>";
            o[1] = o[1].replace("/>", ">");
    ...
    function parse_xlml_xml(d, opts) {
        ...
        while ((Rn = xlmlregex.exec(str)))
            ...
            case "Data":
                ...
                if (Rn[1] === "/")
                    parse_xlml_data(str.slice(didx, Rn.index),
                        ss,
                        dtag,
...
// 2.3.2
    function parse_workbook(blob, options) {
                ...
                case "String":
                    {
                        if (last_formula) {
                            ...
                            if (options.cellFormula)
                                temp_val.f = "=" +
                                    stringify_formula(last_formula.formula, range, last_formula.cell, supbooks, opts);
                            if (temp_val.XF) safe_format_xf(temp_val, options, wb.opts.Date1904);
        ...
        var sheetnamesraw = opts.biff === 2
            ? ["Sheet1"]
            : Object.keys(Directory).sort(function(a, b) { return Number(a) - Number(b); })
