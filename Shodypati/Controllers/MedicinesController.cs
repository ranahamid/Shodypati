...
namespace Shodypati.Controllers
{
    ...
    public class MedicinesController : BaseController
    {
            ...
            if (entity.MedicineName != null)
                foreach (var item in entity.MedicineName)
                {
                    ...
                    if (!string.IsNullOrEmpty(item))
                    {
                        ...
                        if (entity.Price[i] != 0) price = entity.Price[i];
        ...
        public void DeleteFromMedicineTbl(Guid userId)
        {
            ...
            foreach (var item in query.ToList()) Db.MedicineListTbls.DeleteOnSubmit(item);
        ...
        public ActionResult Create(Medicine entity)
        {
            if (ModelState.IsValid)
            {
                ...
                var medicinelist = Db.MedicineListTbls.Where(x => x.UserId == entity.UserId).Select(x =>
                    new MedicineList
                    {
                        MedicineName = x.MedicineName,
                        Price = x.Price,
        ...
        public void SaveToMedicineList(string medicineName, string quantity, int? price, DateTime? finishTime,
            Guid userId)
        {
        ...
        public ActionResult Delete(string Id)
        {
            ...
            var entity = new Medicine {UserId = userId};
            var medicinelist = Db.MedicineListTbls.Where(x => x.UserId == userId).Select(x => new MedicineList
            {
                MedicineName = x.MedicineName,
                Price = x.Price,
